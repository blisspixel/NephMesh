...
- Keys are managed as declared state, so key rotation becomes a policy the reconciliation loop enforces rather than a manual re-flash of every radio (the operator already reconciles channels and their Secret-backed keys; coordinated fleet rotation is a day-2 operation designed in `docs/plans/key-rotation-and-epochs.md` and not built yet).
...
## What if everything is not just down but under attack: jamming, invaders, someone already inside?

That is the case the design is most paranoid about, and it takes two kinds of paranoia at once. Survivability paranoia, the older one from the packet-switching lineage above, assumes the infrastructure will be lost: the mesh routes around damaged nodes, prefers cellular or cloud when they exist and falls to an off-grid mesh when they do not, and never makes the field depend on the control plane, so there is no single thing to knock out. Security paranoia, the assume-breach kind, assumes an adversary is already inside: channels are encrypted, the operator runs at least privilege with no cluster-wide Secret access and no default keys, secret material is wrapped so it cannot leak through a log, and every one of those controls is proven by a test that starts from a compromised position rather than trusted because the design says so (the threat model and the assume-breach tests, for example admission that refuses a hostile resource, a golden test that fails on any extra RBAC grant, and a proof that a broker password never reaches a log). The two paranoias reinforce each other: a layer meant to keep confidential messages moving when everything around it, including a hostile actor, is trying to stop it.

The honest limits matter here, because paranoia without honesty is theatre. A radio transmits by design, so the fact that you transmitted, and roughly where you are, is detectable and direction-findable; NephMesh protects the content of a private channel, not the fact that a transmission happened. A shared channel key means any member can impersonate any other within that channel. The SDR side stays receive-only, so nothing here raises power or keys a transmitter as a side effect. These are properties of the medium, named plainly in the threat model rather than hidden. What the project aims to give you when the carrier is gone and the situation is adversarial is confidential content over a survivable path, honestly bounded, not invisibility.
