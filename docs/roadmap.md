...
- [ ] Reconcile the remaining config surface on real hardware with an actual apply (which reboots the device): verify modemPreset, role, owner, and channels (PSKs from Secrets) round-trip over serial the same way they do against the sim, with the bounded-apply guard. viaGateway transport (today a graceful "unsupported" that reports unreachable)
