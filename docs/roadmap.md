...
- [ ] Verify the remaining fields round-trip over serial on real hardware the way owner does (modemPreset and role are validated against `meshtasticd --sim` in the integration test, and channels are validated against the sim over TCP; a physical-board apply of each is the outstanding hardware confirmation), and pair the operator-on-hardware milestone with spectrum sensing for the full 0.4 gate. viaGateway transport is still a graceful "unsupported" that reports unreachable
