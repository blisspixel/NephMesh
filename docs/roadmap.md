...
## What comes next (recommended order, reviewed 2026-08-11)

The engineering is ahead of the packaging and the proof, and the gap widened since the last review: a hardware-free resilience measurement harness now exists and has demonstrated control-plane independence and airtime-commons survival with numbers (`demo/resilience`, items 2 below), the spectrum exporter is wired into Prometheus in-cluster, two adversarial bug-hunt rounds hardened the core, and the design frontier was pressure-tested into a gated build plan ([`design/road-to-safe-autonomy.md`](design/road-to-safe-autonomy.md)) and a north-star framing ([`design/north-star.md`](design/north-star.md)). None of that changes the near-term order, and by design it adds zero roadmap items: the conclusion is unchanged and now better evidenced, the next work is to make what exists installable and to prove the core claim, not to add capability. The intent-layer frontier stays frozen at the shipped report-only compiler; its own gated order (when it is unfrozen) is in the build plan, whose first rung is a report-only assurance loop, not the capsule.
...
Held frozen until the above lands: actuation from `CommunicationIntent`, the autonomy stack (safety kernel, signed capsules, degrading lease, rejoin), an enforceable `ChannelBudget` admission gate, a second radio driver, and Phases 6 and 7. Over-scoping into that frontier before the core is packaged and proven is the doctrine's named first failure mode; the report-only compiler and the closed-loop proof of concept already signal the direction convincingly. When it does unfreeze, the sequenced, gated order is [`design/road-to-safe-autonomy.md`](design/road-to-safe-autonomy.md), not this list: the veto is built first against an in-memory struct, the signed wire format is last, and model-checking gates any autonomous action.

Two small correctness gaps found by the 2026-08-11 adversarial review are near-term hygiene, not frontier, and fold into finishing the core (item 3): day-2 rotation of only the MQTT password on an otherwise-converged node is not yet detected (it needs a stored password hash, mirroring the channel path), and channel reconcile without the bundled config exporter reboot-loops on the stock export format. Both are tracked; neither blocks the packaging-and-proof work above.
