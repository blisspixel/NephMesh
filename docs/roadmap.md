...
- A Linux USB host (arm64 SBC class): radios and the HackRF attach here, not on the Windows PC. That host may also run a local LLM. Bench work must not reboot it or disturb unrelated services. Hostnames and addresses stay out of the repo; scripts take `SENSOR_SSH`.
...
- A Meshtastic handheld (ESP32-S3 class, USB CDC `303a:1001`): serial path for the operator. Owner, modemPreset, and channels have round-tripped over serial.
- A Linux-native USB LoRa stick (CH341 USB-SPI `1a86:5512`, MeshToad class): `meshtasticd` gateway radio. Not a COM port. Bench: `docs/plans/meshtoad-gateway-bench.md`.
- HackRF Pro (`1d50:6089`): the spectrum sensor, on the Linux USB host. Receive-only by default; it is not a certified Part 15 transmitter, so over-the-air transmit only in authorized contexts (see Phase 6 note).

Nothing needs to be purchased. A cheap RTL-SDR remains optional if a site needs a dedicated sensor while the HackRF is busy elsewhere.

### How far the roadmap goes with zero devices plugged in

Most of it. Hardware-free on the Windows PC alone: all of Phase 0, all of Phase 1 (simulated radios), all of Phase 3 (packaging and Porch), most of Phase 4 (the operator develops and tests against `meshtasticd -s`), the Phase 7 cellular leg (UERANSIM is a simulator), and all CI forever (a project rule). The physical flagship surface (serial and TCP, channels, role, two-node LoRa text) has been confirmed on owned hardware; see `docs/plans/meshtoad-gateway-bench.md`. Devices are still required for: in-cluster USB device-plugin, a Porch wrap of the RF demo, Phase 5's second physical site (rehearsable with two kind clusters), and Phase 6's real-RF closed loop. 0.2 already shipped hardware-free.
...
1. Channels and PSKs on real hardware, which finishes Phase 4. Decode the device's
   ...
   feature. Status: done in software, on `meshtasticd --sim`, and on owned hardware
   (gateway TCP and handheld serial). See Phase 4 and
...
- Continuous hardening: server-side apply for status, the NetworkPolicy cross-namespace
  ...
  controller-runtime v0.24 upgrade; agent-native packaging as an
  [Agent Plugins](https://agent-plugins.org/) wrap around the shipped
  `nephmeshctl` / `nephmesh-mcp` slice (not a Claude-only skill tree); and the
...
## Phase 2: Real radios and spectrum sensing ($0, uses owned hardware) (in progress; T-Deck, MeshToad, and SDR slices done, device-plugin and Git wrap open; depends only on Phase 1)
...
- [x] USB serial transport, validated against a real board. First real-hardware contact (2026-08-05): the operator's `Converge` loop ran against a physical Meshtastic handheld over USB and reached `Ready` read-only. Real-hardware testing surfaced that the CLI's `--export-config` hangs over serial (it re-requests config over admin messages that time out on the device; `--info` and the connect-streamed config are reliable), so the device client uses a bundled exporter (`hack/mesh-export.py`) reading the connect-streamed config for both serial and TCP
...
- [x] Verify the remaining fields on real hardware. modemPreset on the T-Deck over serial (2026-08-09). Channels over TCP on the MeshToad, role ROUTER then CLIENT on the MeshToad, and channels over serial on the T-Deck (2026-08-13): each apply reached Ready with a key-hash or role match and was restored. viaGateway is still a graceful "unsupported" that reports unreachable. Replay: `demo/meshtoad-gateway/`
...
The repo should be as easy for AI agents to work on as for humans, and eventually natural language should be a front door to intent. Two external formats, both new in mid-2026, are the portable box for that, used where they fit and not as a reason to grow surface:

- [Agent Plugins 1.0.0](https://agent-plugins.org/specification) (Working Draft): one directory with `plugin.json`, `skills/` (`SKILL.md` per [Agent Skills](https://agentskills.io/specification)), and `mcp.json`. It is a package format only. It does not install, sandbox, or authorize. NephMesh already has the two portable components (stdio `nephmesh-mcp`, plus `nephmeshctl` and `AGENTS.md`) and wraps them as `agent-plugin/`. Skills live in that layout, not a client-only `.claude/skills/` tree.
- [Open Knowledge Format v0.2](https://cloud.google.com/blog/products/data-analytics/okf-v0-2-adds-trust-signals): markdown plus YAML frontmatter with optional `generated` / `verified` / `sources` / `stale_after` / `status`, and attested computations. That is the same split NephMesh already wants (who wrote it, who checked it, is it still true, was the number computed the sanctioned way). Use it on agent-written or bench-derived notes when they appear, not as a rewrite of `docs/`.

The agent still proposes; the reconcile loop still enforces; the agent is never the control loop (ADR 0002).

- [x] AGENTS.md (and CLAUDE.md pointing to it) with project conventions and the facts agents commonly get wrong
- [x] Offline MCP and CLI (`nephmesh-mcp`, `nephmeshctl plan` / `spectrum` / `advise`): report-only, no cluster, no radio write
- [x] An Agent Plugins package around those binaries (`agent-plugin/`: `plugin.json` + `mcp.json` + skills for plan-intent, operator-demo, meshtoad-bench). Replaces the earlier `.claude/skills/` line. It is a package format, not a new write path.
- [ ] Natural language to intent (Phase 3 onward): a skill that turns "give site X a mesh gateway on MediumSlow with MQTT uplink" into a proposed PackageRevision in Porch. The human approves via Porch's lifecycle
- [ ] Richer MCP (Phase 5 onward): live mesh and spectrum state (nodes, neighbor counts, band occupancy) so an agent can observe and draft intent. Observation only until ADR 0002
