...
- [x] Reconcile modem preset and device role, each field's export path verified to round-trip against `meshtasticd --sim` in the integration test (so it cannot silently never-converge). Region, modem preset, role, and MQTT are now managed
- [ ] Reconcile owner and channels (channel PSKs from Secrets, with the redaction the threat model requires), verified the same way; publish the operator image to a registry with a digest so the package deploys end to end (release step); confirm behavior on a physical board
