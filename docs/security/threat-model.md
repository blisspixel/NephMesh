...
- **The receive-only SDR as an out-of-band trust anchor.** Because it is not a mesh member, a compromised insider holding the shared PSK cannot lie to it: it can reconcile what nodes claim against what was actually transmitted (duplicate node id, impossible mobility, Sybil). The transmit interlock keeps this passive. Scope the claim honestly, though: the SDR is unspoofable by a PSK-holding insider but fully spoofable by an RF-adjacent adversary who holds a transmitter, since that adversary manufactures the very energy the SDR hears. Worse, the radio's own airtime figures and the SDR's occupancy are both functions of the same medium the RF adversary controls, so treating "airtime high and SDR occupancy high agree" as corroboration is two thermometers in the same fire. The genuinely independent corroborator against the RF-adjacent adversary is the application delivery ratio authenticated by the envelope above, not the RF-derived signals. See [`../design/road-to-safe-autonomy.md`](../design/road-to-safe-autonomy.md) section 5.1.
