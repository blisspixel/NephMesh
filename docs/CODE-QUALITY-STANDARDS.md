...
- **envtest** is the primary controller-test tier (a real API server and etcd), because the fake client does not reproduce CRD validation, optimistic-concurrency conflicts, status subresource semantics, or watch timing. The fake client stays for pure branch-logic tests. Implemented: `internal/controller/envtest_test.go` (build-tagged, run in a dedicated CI job) proves admission rejects hostile resources, and the finalizer lifecycle and reconciler idempotency against a real server.
...
- **No secret in logs, status, or events:** inject a sentinel PSK through the reconcile and device-apply path with a captured logger and assert it never appears anywhere. Secret material is wrapped in a redacting type (`internal/secret`) whose String, GoString, MarshalJSON, and MarshalLog all render a placeholder, so a stray log or error cannot disclose it; only an explicit Reveal at the config-write point returns the bytes. Implemented for the MQTT broker password: `internal/controller/secret_logs_test.go` drives a sentinel password through a full reconcile and asserts it reaches the device config but never a log line or the stored resource, and the Secret is read with a namespaced get only (a Role, never a cluster-wide grant), asserted by the RBAC golden test.
...
- **RBAC stays least-privilege:** a golden test asserts the operator's effective permissions equal an expected allowlist (any new grant fails the test), plus a policy rule forbidding wildcards and secret-write. Implemented: `internal/controller/rbac_policy_test.go` reads the shipped ClusterRole and fails on any rule outside the allowlist, any Secret grant, wildcard, or escalation verb; it is proven against a planted grant and guards the earlier removal of the cluster-wide Secret rule.
- **Hostile custom resources are rejected at admission** (envtest): a leading-dash host, oversized fields, unicode tricks, an out-of-range port, or two transports set at once must all be refused by the pattern, length, and CEL validations. Implemented in `internal/controller/envtest_test.go`: six hostile mutations of a valid resource are each refused with an Invalid error by a real API server, and the valid resource is accepted.
- **No default credentials in any shipped package:** a policy test asserts the Meshtastic default channel PSK and the default broker credentials never appear in a package meant for real use. Implemented: `hack/check-secrets.sh` (wired into CI) fails on the full default channel key, the public broker host, or its default account embedded as a value in `packages/` or a demo manifest (comments and docs that name them as facts are excluded); proven against a planted credential.
...
These are tracked as prioritized roadmap items under Phase 4 hardening rather than done all at once, so each lands with its own validation. Already in: the race detector and `govulncheck` in CI, ShellCheck, SHA-pinned Actions with a gate and Dependabot, a checksum-verified kpt download, the fuzzers, the RBAC least-privilege golden test, the envtest controller tier (hostile-CR admission rejection plus finalizer and idempotency proofs), the redacting-secret path with the no-secret-in-logs proof and a namespaced Secret Role, and the fix for the production logger mode. The tier order for what remains, highest leverage first: server-side apply for status, reproducible-build flags and digest-pinned bases, then the last assume-breach control-proving test (the NetworkPolicy cross-namespace proof), then SBOM and signing when publishing begins.
